...
> **Governance decision (2026-04-02 - #TAR-7301)** Yusuf: read canonicalization: account and service_class via str(...).strip().lower() (empty -> 'unknown'); read_id and note collapse internal whitespace, and the collapsed read_id is the identity a read is deduplicated on and the one the bill reports; consumption_kwh and peak_demand_kw coerce via int(str(value).strip()), else int(float(...)), else 0, and a negative result clamps to 0; period_start and period_end are ISO YYYY-MM-DD and a read whose either date will not parse is DROPPED before deduplication and counted in dropped_read_count
Reviewers should reconcile behaviour questions against #TAR governance decisions rather than chat excerpts.
